...
void sendCollectionToServer(){
  ...
  while(it != scansMap->end()){
    ...
    for(size_t i = 0; i<(*scans).size(); i++){
      ...
      size_t needed = snprintf(NULL, 0, "{\"uuidBeacon\":\"%s\",\"majorVersion\":%d,\"minorVersion\":%d,\"idDevice\":\"%s\",\"timePosition\":%ld,\"latitudeDevice\":0.000000,\"longitudeDevice\":0.000000,\"distanceBeacon\":%.2f}",
                      uuid, (*scans)[i].beacon.major, (*scans)[i].beacon.minor, temp, time, (*scans)[i].beacon.distance);
      json = new char[needed +1];
      sprintf(json, "{\"uuidBeacon\":\"%s\",\"majorVersion\":%d,\"minorVersion\":%d,\"idDevice\":\"%s\",\"timePosition\":%ld,\"latitudeDevice\":0.000000,\"longitudeDevice\":0.000000,\"distanceBeacon\":%.2f}",
                      uuid, (*scans)[i].beacon.major, (*scans)[i].beacon.minor, temp, time, (*scans)[i].beacon.distance);
